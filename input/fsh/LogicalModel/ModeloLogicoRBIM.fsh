...
* identificadorInforme 1..1 SU Identifier "Corresponde a una identificación única del informe de biopsia del establecimiento que analiza la muestra."
...
* solicitud 1..1 SU BackboneElement "Corresponde a la solicitud de biopsia del paciente, la cual contiene los datos clínicos y antecedentes relevantes para el análisis de la muestra."
  ...
  * tipoProcedimiento 1..* SU CodeableConcept "Se refiere al método específico empleado para obtener la muestra de tejido o células que será analizada en el laboratorio. Este campo describe el procedimiento realizado, lo cual permite al equipo médico entender el método de extracción y su contexto clínico, ambos importantes para interpretar correctamente los resultados."
  * hipotesisDiagnostica 1..* SU CodeableConcept "Diagnóstico inicial planteado por el médico tratante basado en los signos, síntomas y antecedentes del paciente, que orienta las pruebas diagnósticas necesarias para confirmar o descartar la enfermedad sospechada. Esta hipótesis permite planificar el abordaje clínico mientras se espera el diagnóstico definitivo."
  ...
  * antecedentesClinicosRelevantes 0..* SU string "corresponde a la información clínica relevante del paciente que se debe considerar para el análisis de la muestra, como antecedentes médicos, síntomas actuales, tratamientos previos o cualquier otro dato que pueda influir en la interpretación de los resultados."

* muestra 1..1 SU BackboneElement "Corresponde a la muestra de tejido o células que se analiza en el laboratorio de anatomía patológica, la cual contiene las características macroscópicas y microscópicas del tejido extraído."
  * identificadorDeMuestra 1..1 SU Identifier "Cantidad de muestras analizadas."
...
/// Etapificación
* categoriaTNM 0..1 SU BackboneElement "Clasificación TNM"
